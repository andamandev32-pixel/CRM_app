...
**Version 1.3** | กุมภาพันธ์ 2568
**เพิ่มจาก v1.2:** Module 11 (Project Management), Module 12 (Product Owner Workspace), อัปเดต role และ Dashboard ทุก Role, อัปเดต SETUP Sheets (33 → 46 sheets)
...
## SETUP PROMPT (รันก่อนทุกอย่าง) — v1.3
...
User Roles (6 roles):
- sales       : พนักงานขาย — ใช้ Module 1–10
...
## FINAL INTEGRATION PROMPT — v1.3 (รันหลังสุด)

```
Finalize the Sale Tracking System prototype v1.3 by integrating all 12 modules and 6 user roles.
...
3. Update initializeSheets() — 46 sheets total:
   - All previous 33 sheets from v1.2
   - New sheets: Projects, Milestones, ProjectIssues, DeliveryLogs, ProjectMeetings,
     CustomerAssignments, DevBacklog, FeasibilityLogs, RequirementThreads, ReleaseNotes,
...
5. Update runDailyAlerts() — complete v1.3 list:
   checkOverdueFollowUps()
   checkTrialProgressDue()
   checkTrialEndingSoon()
   checkTrialEndedNoEval()
...
6. Add autoCreateProjectFromOpportunity() trigger:
   - Called inside updateOpportunityStage() when newStage = S6
   - Creates draft Project row linked to opportunityId + customerId
   - Notifies manager to assign PM

7. Add WinLoss prompt when Opportunity closes:
   - When saving opportunity with stage = S6 or manually closing as Won/Lost
   - Redirect to WinLossForm.html after save
   - WinLossLog row links back to opportunityId

8. Update CustomerProfile.html — add 3 new tabs:
...
9. Update GAS manifest (appsscript.json):
   - Keep Gmail scope: "https://www.googleapis.com/auth/gmail.send"

10. Update README.html:
    - Total: 46 Sheets | 12 Modules | 6 Roles | 40+ หน้าจอ
    - URL parameter role-testing guide (all 6 roles)
    - Gmail scope authorization step
    - Trigger setup note (setupTriggers() once)
...
### ลำดับการรัน Prompt
1. `SETUP v1.3` → สร้างโครงสร้างทั้งหมดก่อน (46 sheets)
2. `MODULE 1–12` → รันทีละโมดูลตามลำดับ
3. `FINAL INTEGRATION v1.3` → รันสุดท้ายเพื่อเชื่อมทุกอย่าง
...
- ถ้า IDE ตัด context ให้เริ่มด้วย: `"Continue Sale Tracking System v1.3. SETUP done (46 sheets). Now building [MODULE X] for role [ROLE]."`
...
- Google Sheets 46 sheets — เพียงพอสำหรับทีม 20–30 คน ทุก role รวมกัน
- เมื่อพร้อม Production: migrate ไป Cloud SQL / Firebase + OAuth2 + Role Management จริง

---

## ตารางสรุป Role Matrix v1.3
...
**รวม 46 Sheets / 12 Modules / 6 Roles / 40+ หน้าจอ**

---

*เอกสารนี้จัดทำสำหรับ Sale Tracking System — HIS & Queue Management*
*Version 1.3 | กุมภาพันธ์ 2568*
*เพิ่ม: Module 11 (Project Management), Module 12 (Product Owner Workspace)*
